...
- `/stats` : 統計 / 成長確認
- `/settings` : 全体設定
- `/sessions/[sessionId]` : 保存済みセッション詳細

補足:
- トレーニングモードは route segment で分け、`/train/distance` と `/train/keyboard` をそれぞれ独立した開始入口にする
- `train` ルート配下では、各モードの 1 ルート内で「設定 → 出題 → フィードバック → 結果」の表示状態を切り替える
...
- 完了済み問題のみを集計対象とする
- 残り時間はセッション中に表示し、0 到達時は即座に結果表示へ遷移する
- 回答 0 件で終了した場合も結果画面は表示できるが、保存は行わない

### 音声失敗時の扱い
- 基準音 / 問題音 / リプレイの再生失敗は画面内 notice として示す
- 再生失敗でも回答、次問進行、手動終了は継続できる
- 音声失敗だけを理由にセッション全体を中断しない
- 回答後の効果音は `正解 / 惜しい / 不正解` の 3 段階とし、`惜しい` は正解に対して `±1 半音` の回答に対応する
...
- 日次推移グラフ
  - 主グラフ: スコア
  - 補助グラフ: 誤差、回答時間、正答率、問題数
- 音程別パフォーマンス
- 上下方向の比較
- 回答が高め / 低め / 一致の傾向
- 最近のセッション一覧とセッション詳細導線

### 実装方針
- ログイン必須
- MVP では期間フィルタなし
- 初期実装は都度集計ベースとし、重くなったら日次集約テーブルを後続検討する
- 集計要件と初期 UI 表示ブロックを分けて扱い、将来は同じ集計を別 UI に再利用できるようにする

## 4.5 設定画面 `/settings`
### 目的
- 全体設定を変更し、モード別の保存済み前回設定を確認 / リセットする
...
- 保存失敗時は現在表示値を保ったまま notice と再試行手段を出す

### モード別前回設定
- `distance` / `keyboard` それぞれの last-used config を独立して表示する
- 各モードの保存済み設定は canonical default に個別リセットできる
- モード別前回設定のリセットは全体設定へ影響しない

## 4.6 セッション詳細画面 `/sessions/[sessionId]`
### 目的
- 保存済みセッションの概要、設定スナップショット、回答結果を確認する
